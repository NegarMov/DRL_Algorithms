...
class ReplayBuffer():
    ...
    def sample(self, sample_size):
        ...
        rewards = np.array([t[3] for t in mini_batch], dtype=np.float32)[:, None]
        terminateds = np.array([t[4] for t in mini_batch], dtype=np.float32)[:, None]

        return (
            torch.as_tensor(obs, dtype=torch.float32),
            torch.as_tensor(acs, dtype=torch.float32),
