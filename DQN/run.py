    ...
    for e in tqdm(range(params['episodes'])):
        ...
        while not episode_over:
            ...
            if random.rand() < params['e']:
                ...
                with torch.no_grad():
                    ac = target_dqn(encode_ob(ob, ob_dim)).argmax().item()
        ...
        if len(replay_buffer) > params['batch_size'] and np.sum(reward_per_episodes) > 0:
            ...
            for ob, ac, new_ob, reward, terminated in mini_batch:
                ...
                if terminated:
                    ...
                    with torch.no_grad():
                        if params['ddqn']:
                            policy_ac = policy_dqn(encode_ob(new_ob, ob_dim)).argmax()
                            target = torch.FloatTensor(
                                reward + params['df'] * target_dqn(encode_ob(new_ob, ob_dim))[policy_ac]
                            )
                        else:
                            target = torch.FloatTensor(
                                reward + params['df'] * target_dqn(encode_ob(new_ob, ob_dim)).max()
                            )
                target_q = target_dqn(encode_ob(ob, ob_dim)) 
                ...
                policy_q_values.append(policy_dqn(encode_ob(ob, ob_dim)))
            ...
            params['e'] = max(params['e'] - 1 / params['episodes'], 0)
...
def evaluate(params):
    ...
    for e in range(params['episodes']):
        ...
        while not episode_over:
            ...
            with torch.no_grad():
                ac = policy(encode_ob(ob, ob_dim)).argmax().item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_buffer_size', type=int, default=1000, help='Maximum capacity of the replay buffer')
